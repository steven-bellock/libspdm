...
static void rsp_encapsulated_response_ack_case2(void **State)
{
    ...
    spdm_context->encap_context.payload_buffer_max_size =
        sizeof(m_libspdm_local_certificate_chain);
    if (!libspdm_read_responder_public_certificate_chain(m_libspdm_use_hash_algo,
                                                         m_libspdm_use_asym_algo, &data,
                                                         &data_size,
...
#if LIBSPDM_SEND_GET_ENDPOINT_INFO_SUPPORT
/**
 * Test 27 (DELIVER_ENCAPSULATED_RESPONSE) the Requester returns a valid ENDPOINT_INFO response that
 ...
 **/
static void rsp_encapsulated_response_ack_case27(void **State)
{
    libspdm_return_t status;
    libspdm_test_context_t *spdm_test_context;
    libspdm_context_t *spdm_context;
    ...
    spdm_context = spdm_test_context->spdm_context;
    /* No handler case is defined for this case_id, so the test fails if the handler is consulted. */
    spdm_test_context->case_id = 0xA7;
    m_case_id = spdm_test_context->case_id;
    ...
    ep_info_data_len = sizeof(m_libspdm_endpoint_info);
    spdm_context->encap_context.payload_buffer = m_libspdm_endpoint_info;
    ...
    spdm_context->encap_context.payload_buffer_max_size = sizeof(m_libspdm_endpoint_info);
}
#endif /* LIBSPDM_SEND_GET_ENDPOINT_INFO_SUPPORT */
